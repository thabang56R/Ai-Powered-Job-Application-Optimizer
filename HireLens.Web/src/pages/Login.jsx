...
export default function Login() {
  ...
  return (
    <div style={wrap}>
      <div style={card}>
        <div style={{ fontSize: 22, fontWeight: 900 }}>HireLens</div>
        <div style={{ opacity: 0.7, marginTop: 6 }}>Sign in to recruiter portal</div>
        ...
        <form onSubmit={onSubmit} style={{ marginTop: 16, display: "grid", gap: 10 }}>
          ...
          <button style={btn} disabled={loading}>{loading ? "Signing in..." : "Sign in"}</button>
        </form>

        <div style={{ marginTop: 14, fontSize: 13, opacity: 0.8 }}>
          No account? <Link to="/register" style={{ color: "white" }}>Create one</Link>
        </div>
      </div>
    </div>
  );
